...
def create_test_scene():
    ...
    pose1 = CameraPose(
        position=np.array([-3.0, 0.0, 1.0]), orientation=quat1, timestamp=0.0
    )
    poses.append(pose1)
    ...
    pose2 = CameraPose(
        position=np.array([0.0, 0.0, 4.0]), orientation=quat2, timestamp=1.0
    )
    poses.append(pose2)
    ...
    pose3 = CameraPose(
        position=np.array([-2.0, -2.0, 2.0]), orientation=quat3, timestamp=2.0
    )
    poses.append(pose3)
...
def main():
    ...
    # Create visualizer in static mode (non-interactive)
    visualizer = OpenCVSceneVisualizer(
        image_width=1280, image_height=720, background_color=(30, 30, 30),
        interactive=False  # Static viewing mode
    )
